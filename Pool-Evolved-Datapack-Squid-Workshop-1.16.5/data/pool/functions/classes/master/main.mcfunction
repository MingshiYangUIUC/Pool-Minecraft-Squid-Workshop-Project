...
execute at @s[scores={swPool_v=1..}] run tag @e[tag=swPool_pool,tag=!swPool_fake,distance=0.001..2.25,sort=nearest,limit=4] add swPool_near

execute at @s[scores={swPool_v=1..}] if entity @e[tag=swPool_fake,distance=..1.5] run tag @e[tag=swPool_fake,distance=0.001..1.5,sort=nearest,limit=3] add swPool_near
