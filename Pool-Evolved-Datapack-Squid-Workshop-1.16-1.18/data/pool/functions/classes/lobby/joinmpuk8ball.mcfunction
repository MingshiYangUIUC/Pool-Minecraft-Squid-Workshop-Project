...
execute if entity @s[tag=swPool_empty] run execute if entity @a[distance=0.001..,tag=swPool_wait_uk8ball] run tellraw @a[tag=!swPool_mute,tag=swPool_EN] [{"text":"[MP UK 8 Ball] "},{"selector":"@a[tag=swPool_wait_uk8ball,limit=1,sort=furthest]"},{"text":", "},{"selector":"@a[tag=swPool_wait_uk8ball,limit=1,sort=nearest]"},{"text":", "},{"text":"<BEGIN!>","color":"white","underlined":true,"clickEvent":{"action":"run_command","value":"/function pool:classes/uk8ball/start_helper"}}]
execute if entity @s[tag=swPool_empty] run execute if entity @a[distance=0.001..,tag=swPool_wait_uk8ball] run tellraw @a[tag=!swPool_mute,tag=swPool_CN] [{"text":"[多人英式八球] "},{"selector":"@a[tag=swPool_wait_uk8ball,limit=1,sort=furthest]"},{"text":"，"},{"selector":"@a[tag=swPool_wait_uk8ball,limit=1,sort=nearest]"},{"text":"，"},{"text":"<开始！>","color":"white","underlined":true,"clickEvent":{"action":"run_command","value":"/function pool:classes/uk8ball/start_helper"}}]
