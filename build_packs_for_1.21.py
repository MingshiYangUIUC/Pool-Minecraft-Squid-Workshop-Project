...
for v in all_versions:
    # desired version
    pool_major_version, pool_minor_version = v
    ...
    if modify_height_armor_stand:
        print('  Fixing armor stand y position in Datapack for 1.21.2+...')
...
print('Generating 1.21.X Master Packs...')

def scanfile(dir_scan_str, dir_default_str):
    dir_default = Path(dir_default_str)
    dir_scan = Path(dir_scan_str)
    ...
    if not dir_default.exists() or not dir_scan.exists():
        print("Error: One or both directories do not exist. Please check your paths.")

    # Scan all files in the scanning directory
    for scan_file in dir_scan.rglob('*'):
        if scan_file.is_file():
            # Get the path of the file relative to the dir_scan base
            relative_path = scan_file.relative_to(dir_scan)
            
            # Build the exact path where this file should be in dir_default
            default_file = dir_default / relative_path
            
            # Check if the file exists in the default directory
            if default_file.exists() and default_file.is_file():
                # Compare contents (shallow=False forces a byte-by-byte comparison)
                if filecmp.cmp(scan_file, default_file, shallow=False):
                    identical_files.append(relative_path)
                else:
                    different_files.append(relative_path)
            else:
                missing_in_default.append(relative_path)

    # --- Output the Results ---
    print(f"--- Scan Results ---")
    print(f"Total files scanned: {len(identical_files) + len(different_files) + len(missing_in_default)}\n")
    return identical_files, different_files, missing_in_default
...
if os.path.isdir(datapack_dir_target_str):
    shutil.rmtree(datapack_dir_target_str)

os.makedirs(datapack_dir_target_str,exist_ok=True)
shutil.copytree(datapack_dir_default_str, os.path.join(datapack_dir_target_str,'data'))
shutil.copy(os.path.join(datapack_dir_default_str,'..','pack.png'),datapack_dir_target_str)
shutil.copy('.mcmeta_datapack_1.21.mcmeta',os.path.join(datapack_dir_target_str,'pack.mcmeta'))

for dir_scan_str, dir_overlay_name in datapack_data:
    
    dir_scan = Path(dir_scan_str)

    overlay_base = datapack_dir_target / dir_overlay_name 

    # Assume these variables come from your scanfile() function
    identical_files, different_files, missing_in_default = scanfile(dir_scan_str, datapack_dir_default_str)

    # Combine "different" and "missing" because both are needed for the overlay
    files_to_copy = different_files + missing_in_default

    print(f"Datapack... Starting copy of {len(files_to_copy)} files to: {overlay_base}")
    os.makedirs(overlay_base,exist_ok=True)
    for rel_path in files_to_copy:
        # 1. Define the source and destination paths
        source_path = dir_scan / rel_path
        target_path = overlay_base / 'data' / rel_path
        
        # 2. Create the deep directory structure if it doesn't exist
        # target_path.parent gives us the folder the file will sit in
        target_path.parent.mkdir(parents=True, exist_ok=True)
        
        # 3. Copy the file (copy2 preserves metadata like timestamps)
        shutil.copy2(source_path, target_path)
        print(f"Datapack...   Copied: {rel_path}")
...
if os.path.isdir(resourcepack_dir_target_str):
    shutil.rmtree(resourcepack_dir_target_str)

os.makedirs(resourcepack_dir_target_str,exist_ok=True)
shutil.copytree(resourcepack_dir_default_str, os.path.join(resourcepack_dir_target_str,'assets'))
shutil.copy(os.path.join(resourcepack_dir_default_str,'..','pack.png'),resourcepack_dir_target_str)
shutil.copy('.mcmeta_resourcepack_1.21.mcmeta',os.path.join(resourcepack_dir_target_str,'pack.mcmeta'))

for dir_scan_str, dir_overlay_name in resourcepack_data:
    
    dir_scan = Path(dir_scan_str)

    overlay_base = resourcepack_dir_target / dir_overlay_name 

    # Assume these variables come from your scanfile() function
    identical_files, different_files, missing_in_default = scanfile(dir_scan_str, resourcepack_dir_default_str)

    # Combine "different" and "missing" because both are needed for the overlay
    files_to_copy = different_files + missing_in_default

    print(f"Resourcepack... Starting copy of {len(files_to_copy)} files to: {overlay_base}")
    os.makedirs(overlay_base,exist_ok=True)
    for rel_path in files_to_copy:
        # 1. Define the source and destination paths
        source_path = dir_scan / rel_path
        target_path = overlay_base / 'assets' / rel_path

        if '.jpg' in str(rel_path):
            continue
        ...
        # 3. Copy the file (copy2 preserves metadata like timestamps)
        shutil.copy2(source_path, target_path)
        print(f"Resourcepack...   + Copied: {rel_path}")
