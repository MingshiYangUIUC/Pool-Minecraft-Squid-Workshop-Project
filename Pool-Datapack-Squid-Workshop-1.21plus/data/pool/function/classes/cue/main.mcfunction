...
execute if entity @s[tag=swPool_offground] as @e[type=arrow,distance=..3] if score @s swPool_player = @p[tag=swPool_offground] swPool_player run kill @s
...
execute if entity @e[tag=swPool_cue,limit=1] at @s if score @e[type=arrow,distance=..3,limit=1] swPool_player = @s swPool_player run function pool:classes/cue/shoot
