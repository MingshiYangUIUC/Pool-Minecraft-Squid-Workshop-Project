...
execute unless score #bot_turn swMath_V matches 1 run execute if data storage minecraft:swpool cueballreddot as @a[tag=swPool_ballinhand] at @s run give @s minecraft:carrot_on_a_stick[minecraft:item_model="swpool:object_cueball",minecraft:custom_name="\"Cue Ball\""]
execute unless score #bot_turn swMath_V matches 1 run execute unless data storage minecraft:swpool cueballreddot as @a[tag=swPool_ballinhand] at @s run give @s minecraft:carrot_on_a_stick[minecraft:item_model="swpool:object_cueball_clean",minecraft:custom_name="\"Cue Ball\""]

tag @e[tag=swPool_pooltable] add swPool_start

tag @a[limit=1,sort=random,tag=swPool_wait_9ball] add swPool_poolplay

execute if entity @a[tag=!swPool_poolplay] run tag @a[limit=1,tag=swPool_wait_9ball,tag=!swPool_poolplay] add swPool_poolplay
...
execute unless score #bot_turn swMath_V matches 1 run tellraw @a[tag=swPool_spec,tag=swPool_EN] [{"text":"➇ ","color":"white"},{"text":"<Adjust the next strike.>","color":"white","underlined":true,"click_event":{"action":"run_command","command":"/trigger swPool__trigger set 4110906"}}]
execute unless score #bot_turn swMath_V matches 1 run tellraw @a[tag=swPool_spec,tag=swPool_CN] [{"text":"➇ ","color":"white"},{"text":"<调整下一次击球>","color":"white","underlined":true,"click_event":{"action":"run_command","command":"/trigger swPool__trigger set 4110906"}}]
