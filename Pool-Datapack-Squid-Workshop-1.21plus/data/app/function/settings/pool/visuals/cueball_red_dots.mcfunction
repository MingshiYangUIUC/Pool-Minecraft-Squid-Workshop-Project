...
tellraw @s[tag=swPool_EN] [{"text":"➇ Switch back by using /function app:settings/pool/visuals/cueball_clean, and run /function app:get/pool/cueball_helper to get a new cue ball if necessary.","color":"white"}]

tellraw @s[tag=swPool_CN] [{"text":"➇ 已切换为带红点白球，建议不要在球局中使用此功能。","color":"white"}]
tellraw @s[tag=swPool_CN] [{"text":"➇ 如有需要，请用 /function app:settings/pool/visuals/cueball_clean 换回纯色白球，并可用 /function app:get/pool/cueball_helper 获得一颗新球。","color":"white"}]
