...
execute if entity @s[tag=swPool_empty] run execute if entity @a[distance=0.001..,tag=swPool_wait_cn8ball] run tellraw @a[tag=!swPool_mute,tag=swPool_EN] [{"text":"➇ ","color":"white"},{"text":"[MP CN 8 Ball] "},{"selector":"@a[tag=swPool_wait_cn8ball,limit=1,sort=furthest]"},{"text":", "},{"selector":"@a[tag=swPool_wait_cn8ball,limit=1,sort=nearest]"},{"text":", "},{"text":"<BEGIN!>","color":"white","underlined":true,"click_event":{"action":"run_command","command":"/trigger swPool_trigger set 111"}}]
execute if entity @s[tag=swPool_empty] run execute if entity @a[distance=0.001..,tag=swPool_wait_cn8ball] run tellraw @a[tag=!swPool_mute,tag=swPool_CN] [{"text":"➇ ","color":"white"},{"text":"[多人中式八球] "},{"selector":"@a[tag=swPool_wait_cn8ball,limit=1,sort=furthest]"},{"text":"，"},{"selector":"@a[tag=swPool_wait_cn8ball,limit=1,sort=nearest]"},{"text":"，"},{"text":"<开始！>","color":"white","underlined":true,"click_event":{"action":"run_command","command":"/trigger swPool_trigger set 111"}}]
