...
execute if score #tick swPool_var00 matches 0 if entity @e[type=armor_stand,tag=swPool_pool,scores={swPool_v=1..}] at @e[type=armor_stand,tag=swPool_pooltable] run function pool:classes/main/tick_iterate
...
execute if score swPool_gameon swMath_V matches 1 unless entity @e[type=armor_stand,tag=swPool_pool,scores={swPool_v=1..}] as @e[tag=swPool_pooltable,tag=!swPool_start,tag=!swPool_progressed] run function pool:classes/master/idle
scoreboard players add @e[type=armor_stand,tag=swPool_pooltable] swPool_lifetime 1

scoreboard players set @a swPool_crtclk 0

execute if score swPool_gameon swMath_V matches 1 unless score #fastfwd swMath_V matches 1 if score #tick swPool_var00 matches 0 if data storage minecraft:swpool {allowspin:1} as @e[tag=swPool_pool,scores={swPool_T=1..,swPool_v=1..}] run function pool:classes/pose/w2dpdt_iterative

execute if score swPool_gameon swMath_V matches 1 unless score #fastfwd swMath_V matches 1 if score #tick swPool_var00 matches 0 as @e[tag=swPool_potting,type=armor_stand] at @s run function pool:classes/pocketing/animation/loop
