...
execute unless entity @s[scores={swPool_var01=0}] unless entity @s[scores={swPool_var02=0}] at @e[tag=swPool_placer] run summon item_display ~ ~ ~ {Tags:["swPool_new","swPool_cloth"]}
...
execute if score @s swPool_var01 <= @s swPool_var03 if score @s swPool_var02 <= @s swPool_var04 run function pool:classes/table/loop_item_display