...
execute unless score #breakhappen swMath_V matches 0 unless entity @s[tag=swPool_bounce] if entity @s[tag=swPool_col] as @e[type=armor_stand,tag=swPool_pool,tag=swPool_cue,limit=1] at @s run function pool:classes/break_nn/io
