# reset fastfwd scores
#tellraw @a[tag=swPool_debug] [{"text":"evaluating"}]
...
# at the end of tryout loop, score update
scoreboard players add #tryoutloop swMath_V 1
# bestshot = tryoutloop + 1
execute if score @s swPool_shotScore > #bestshotscore swMath_V run scoreboard players operation #bestshot swMath_V = #tryoutloop swMath_V
...
#tellraw @a[tag=swPool_debug] [{"text":"Eval end, next: "},{"score":{"objective":"swMath_V","name":"#tryoutloop"}},{"text":" max: "},{"score":{"objective":"swMath_V","name":"#tryoutloopmax"}}]


# get out of itertry loop
execute if score #tryoutloop swMath_V >= #tryoutloopmax swMath_V run function pool:classes/bot/generic/_obtain_actions_entity_d0_tryout_loopend_10

# or reenter tryout loop
execute if score #tryoutloop swMath_V < #tryoutloopmax swMath_V run function pool:classes/bot/generic/_obtain_actions_entity_d0_tryout_loop_10
