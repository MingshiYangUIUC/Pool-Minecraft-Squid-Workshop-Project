...
# 简介
本数据包为广大我的世界、台球爱好者制作。  
本数据包支持用我的世界Java Edition 1.16+游玩，包括单人和双人游戏。
练习模式，九球，中式八球，英式八球，斯诺克模式已开发完成。 
...
- **请用同样的方法安装[数学数据包](https://github.com/MingshiYangUIUC/Math-Minecraft-Squid-Workshop-Project)。本数据包提供它的1.0.0副本。**
...
🔴 请确认也加载了数学数据包，有时需要再次reload保证成功安装。
...
2. 球的运动看起来不太正常。 \
请确认你已经成功安装了 **Math Datapack**（见上方说明），在执行 `reload` 后你应该能看到来自两个数据包的提示信息。在极少数情况下，如果球桌上同时存在过多的球，适当提高游戏规则中的 `maxcommandchainlength` (新版本：`max_command_sequence_length`) 可能会有所帮助。
