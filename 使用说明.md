# _鱿鱼MC工作室_ 台球数据包
内容创作者：杨铭石。联系方式：mingshi3@illinois.edu
...
- Step 05 将数据包文件夹 "Pool-Evolved-Datapack-Squid-Workshop-1.16-1.18" folder(not the root folder) 放入 .../saves/world/datapacks 文件夹；
...
注意：[旧版台球数据包](https://github.com/MingshiYangUIUC/Pool-Minecraft-Squid-Workshop-Project-Old-Version)和本数据包不兼容；暂不支持多个球桌同时运行；关闭强制Unicode字体功能后体验更佳。
...
# 原理
简化的牛顿力学，包含各种使运算简单的假设。省略了袋角和球之间的摩擦力，以及所有物体的形变等。编写时我没有引用文献，您可以参考基础物理课本或者在互联网搜索“台球物理”。

# 注意
本数据包中球的建模相比真实较大，且包含众多简化的运算以使其在Minecraft中能够运行，故无法期望它完美地还原真实世界中的物理过程及表现形式，请多包涵。

为了方便游玩，简化后的斯诺克[规则](https://zh.wikipedia.org/wiki/%E6%96%AF%E8%AF%BA%E5%85%8B)和英式八球[规则](https://zh-yue.wikipedia.org/wiki/%E8%8B%B1%E5%BC%8F16%E6%B3%A2)被加入了数据包并以文字形式出现，简化后的规则比正式规则宽松。球桌袋口的规格近似于2005年的斯诺克标准，它的图例及简化版可以在资源包贴图文件中找到。

# 目录
## 设置目录
	data/app/functions/settings		   注释
	+---aim                               	   在瞄准时是否放大屏幕
	|       nozoom.mcfunction                          不放大
	|       zoom.mcfunction                            放大
	|
	+---cheating                               作弊设置
	|       allow.mcfunction                           开
	...
	|       impact.mcfunction                          球和库边的系数
	|       rolling.mcfunction                         球滚动的系数
	|       sliding.mcfunction                         球滑动的系数
	|       striking.mcfunction                        球和球杆的系数
	|
	+---language                               语言设置
	...
	|       
	+---snooker/display                        斯诺克游戏的分数显示
	|       dialogue.mcfunction                        从聊天栏推送给玩家
	|       sidebar.mcfunction                         在侧边栏显示（可能与其它计分板冲突）
	|       
	\---version                                地图（世界）版本设置
