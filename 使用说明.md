...
- 在 Minecraft 中，准星（**+**）并不完全等同于玩家在世界坐标中的真实朝向。例如在 1.21.11 版本中，朝向略微偏移（对齐竖线的右侧）。建议适当调整瞄准方向，或在设置中手动调整母球偏移来进行校正。
- 建议使用 Minecraft 1.21 及以上版本进行游玩，因为基于 item display 实现的画面效果会有明显提升。
