...
3. 我无法撤销一次击球。 \
请在设置菜单中允许 `作弊(cheat)`。在对局过程中撤销击球被视为一种“作弊”行为并且初始化时不被允许。
...
---
# 注意事项
1. **球的尺寸大于现实**，这是为了提高可控性和游戏体验。真实尺寸虽然更接近现实，但在 Minecraft 的鼠标灵敏度下极难精准操作球杆。希望您理解这是为了带来更好的游玩体验。相应的，球速会让玩家觉得比现实中低。

2. **请勿未经署名传播本数据包内容**。虽然物理内容大多为通识知识，但本数据包中所有建模与实现均为本人独立完成，未引用具体论文文献。

3. **数据包内各种台球游戏的规则已作简化**，以提升可玩性与易用性。您可以参考：[斯诺克规则](https://zh.wikipedia.org/wiki/%E6%96%AF%E8%AF%BA%E5%85%8B)、[英式八球规则](https://zh-yue.wikipedia.org/wiki/%E8%8B%B1%E5%BC%8F16%E6%B3%A2)。
