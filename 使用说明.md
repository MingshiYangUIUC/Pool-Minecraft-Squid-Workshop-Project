...
2. 球的运动看起来不太正常。 \
请确认你已经成功安装了 **Math Datapack**（见上方说明），在执行 `reload` 后你应该能看到来自两个数据包的提示信息。在极少数情况下，如果球桌上同时存在过多的球，适当提高游戏规则中的 _maxcommandchainlength_ 可能会有所帮助。

3. 我无法撤销一次击球。 \
请在设置菜单中允许 _作弊(cheat)_。在对局过程中撤销击球被视为一种“作弊”行为。

4. 执行 `reload` 之后一切都消失了。 \
请在设置菜单中选择 _keepongoingsession_。

5. 数据包在运行，但效果和视频中不一样。 \
请尝试调整可用的设置，尤其是 _visuals_ 相关选项。需要注意的是，不同的物理参数有时也会导致明显不同的效果。
