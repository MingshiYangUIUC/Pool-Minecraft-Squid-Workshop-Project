...
tellraw @a[tag=swPool_debug] [{"text":"Looping actions, "},{"score":{"objective":"swMath_V","name":"#rank_i"}},{"text":" of "},{"score":{"objective":"swMath_V","name":"#topk"}}]
...
scoreboard players operation #tryoutloopmax swMath_V = n_act_space swPool_C
...
#execute if score n_act_space swPool_C matches 10 if score #rank_i swMath_V < #topk swMath_V as @e[tag=swPool_aim_obj_selected,limit=1] run function pool:classes/bot/generic/_obtain_actions_entity_d0_tryout_loop_10

# rescheduler moved to the end (tryout_loop)