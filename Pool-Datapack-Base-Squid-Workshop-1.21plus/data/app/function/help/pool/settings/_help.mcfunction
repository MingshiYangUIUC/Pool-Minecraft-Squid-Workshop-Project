...
tellraw @s[tag=swPool_EN] [{"text":"➇ ", "color":"gold"},{"underlined":true,"text":"<bot/angle_std>","click_event":{"action":"run_command","command":"/function app:settings/pool/bot/angle_std"}},{"underlined":false, "color":"white","text":"    Adjust the random angle spread of bot shots. Lower values make the bot stronger."}]
tellraw @s[tag=swPool_CN] [{"text":"➇ ", "color":"gold"},{"underlined":true,"text":"<bot/angle_std>","click_event":{"action":"run_command","command":"/function app:settings/pool/bot/angle_std"}},{"underlined":false, "color":"white","text":"    调整电脑玩家出杆的随机角度偏差，数值越低电脑玩家越强"}]
