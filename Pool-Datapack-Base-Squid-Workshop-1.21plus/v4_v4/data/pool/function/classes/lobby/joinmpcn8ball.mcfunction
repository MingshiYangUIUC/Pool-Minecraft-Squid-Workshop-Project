...
execute if entity @s[tag=swPool_empty] run execute if entity @a[distance=0.001..,tag=swPool_wait_cn8ball] run tellraw @a[tag=!swPool_mute,tag=swPool_EN] [{"text":"➇ ","color":"white"},{"text":"[DUO 8 ball] "},{"selector":"@a[tag=swPool_wait_cn8ball,limit=1,sort=furthest]"},{"text":", "},{"selector":"@a[tag=swPool_wait_cn8ball,limit=1,sort=nearest]"},{"text":", "},{"text":"<BEGIN!>","color":"white","underlined":true,"clickEvent":{"action":"run_command","value":"/function pool:classes/cn8ball/start_helper"}}]
execute if entity @s[tag=swPool_empty] run execute if entity @a[distance=0.001..,tag=swPool_wait_cn8ball] run tellraw @a[tag=!swPool_mute,tag=swPool_CN] [{"text":"➇ ","color":"white"},{"text":"[双人八球] "},{"selector":"@a[tag=swPool_wait_cn8ball,limit=1,sort=furthest]"},{"text":"，"},{"selector":"@a[tag=swPool_wait_cn8ball,limit=1,sort=nearest]"},{"text":"，"},{"text":"<开始！>","color":"white","underlined":true,"clickEvent":{"action":"run_command","value":"/function pool:classes/cn8ball/start_helper"}}]
